...
class ParseRamon(BaseParser):
    def __init__(self, initial_data: dict = None) -> List:
    ...
    def exchange_renew_task(self, driver: Any = None,task_url: str = None) -> List:
        if not driver:
            driver = self._get_driver(self.host, self.port, self.usr, self.pwd)
            close = True
        else:
            close = False
        try:
    ...
    def parse_page(self):
        ...
        try:
            ...
            self.get_click_currency(self.currency_from,
                                    driver.find_elements(By.CLASS_NAME, "col-md-3")[0].find_elements(By.CLASS_NAME,
                                                                                                     "row")).click()
            self.get_click_currency(self.currency_to,
                                    driver.find_elements(By.CLASS_NAME, "col-md-3")[1].find_elements(By.CLASS_NAME,
                                                                                                     "row")).click()
